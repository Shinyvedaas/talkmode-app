...
[Versão mais recente](https://talkmode.baryon.ai/download/TalkMode-0.4.2.zip) → descompactar → arrastar para `Applications` → no primeiro arranque o macOS pode dizer "de um programador não identificado", clique com o botão direito → Abrir uma vez.

```bash
curl -L https://talkmode.baryon.ai/download/TalkMode-0.4.2.zip -o TalkMode.zip
