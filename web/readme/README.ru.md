...
[Последний релиз](https://talkmode.baryon.ai/download/TalkMode-0.4.5.zip) → распакуйте → перетащите в `Applications` → при первом запуске macOS может сказать «от неизвестного разработчика», щёлкните правой кнопкой → Открыть один раз.
